...
extern int _numVirtCalls;
extern int _numStaticCalls;
extern int _numAssignments;
extern int _numFieldLoads;
extern int _numFieldStores;
...
retry:
		...
		PORT_ACCESS_FROM_JAVAVM(_vm);
		getMethodName(PORTLIB, _literals, _pc, currentMethodName);
		cout << "GET_FIELD: " << currentMethodName << endl;
		return rc;
	}
	...
	putfield(REGISTER_ARGS_LIST)
	{
		...
		PORT_ACCESS_FROM_JAVAVM(_vm);
		getMethodName(PORTLIB, _literals, _pc, currentMethodName);
		cout << "PUT_FIELD: " << currentMethodName << endl;
		return rc;
	}
	...
	invokevirtualLogic(REGISTER_ARGS_LIST, bool fromBytecode)
	{
		...
		if (J9_UNEXPECTED(NULL == receiver)) {
			...
			if (fromBytecode) {
				...
				PORT_ACCESS_FROM_JAVAVM(_vm);
				getMethodName(PORTLIB, _literals, _pc, currentMethodName);
				cout << "VIRTUAL: " << currentMethodName << endl;
			}
		}
	...
	invokestatic(REGISTER_ARGS_LIST)
	{
		...
		PORT_ACCESS_FROM_JAVAVM(_vm);
		getMethodName(PORTLIB, _literals, _pc, currentMethodName);
		cout << "STATIC: " << currentMethodName << endl;
	...
	invokestaticsplit(REGISTER_ARGS_LIST)
	{
		...
		PORT_ACCESS_FROM_JAVAVM(_vm);
		getMethodName(PORTLIB, _literals, _pc, currentMethodName);
		cout << "STATIC_SPLIT: " << currentMethodName << endl;
